...
email status until they click the link in the confirmation email.`
    ...
    ethAddress: {
      type: 'string',
      required: true,
      description: 'A user\'s ethereum address with which they are linked to an asset on the blockchain.',
      maxLength: 42,
      example: "0xvGhny6uehn0ndhgbgbs9ksjsh0398jjduI8676Te"
    },
...
So, while this \`isSuperAdmin\` demarcation might not be the right approach forever, it's a good place to start.`
    ...
    // settings for cron job
    shouldTransferAsset: {
      type: 'number',
      defaultsTo: -1,
      description: 'An integer in months used to specify after when an asset should be transferred'
    }
